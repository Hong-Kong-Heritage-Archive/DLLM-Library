...
export const CONTENT_RATING_OPTIONS: ContentRatingOption[] = [
  ...
  {
    value: 5,
    labelKey: "contentRating.cat4",
    color: "error",
    svg: "", // No official SVG for Category IV
  },
];
...
/**
 * Returns whether an item needs admin approval before it can be seen by users,
 * based on its contentRating and whether an admin has checked it.
 */
export function needsAdminApproval(
  contentRating: number,
  contentRatingChecked: boolean
): boolean {
  return contentRating >= CONTENT_RATING_CENSOR_THRESHOLD && !contentRatingChecked;
}
